...
async function seedDatabase() {
  try {
    ...
    for (const comment of sampleComments) {
      ...
      // Update comment count for the secret
      const secretIndex = items.findIndex(
        item => item.PutRequest?.Item?.pk === `secret#${secretId}`
      );
    ...
    console.log(`Successfully seeded database with ${sampleSecrets.length} secrets and ${sampleComments.length} comments.`);
  } catch (error) {
